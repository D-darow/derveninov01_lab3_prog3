...
public:
	void set_diametr_of_disk(int diametr_of_disk); // Установка диаметра колеса
	int get_diametr_of_disk(); // Получение диаметра колеса
	void set_width_of_tire(int width_of_tire); // Установка ширины шины
	int get_width_of_tire(); // Получение ширины шины
	wheels(); // Конструктор по умолчанию
	wheels(int diametr_of_disk, int width_of_tire); // Конструктор
	~wheels(); // Деструктор
